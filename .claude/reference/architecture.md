...
- `launcher.mjs`: entry point (`Launch Harness Console.vbs` runs it). Serves the app on 43127, starts `usage/server.mjs` on 4545 (`USAGE_PORT` overrides), replaces a running instance unless it is mid-operation. Before loading the app it fast-forwards the folder to `origin/main` (`selfUpdate`; only on a clean main) and, when main moved, relaunches itself so the new code loads. The footer shows the build and any skip reason via `/api/status` `build`.
