...
// How the shipped config schedules upstream fetches. 0 in usage/accounts.json turns a timer off.
const timer = (p, name) => p.on ? `${name} every ${p.seconds} seconds` : null;
const timers = [timer(F.claudePoll, "Claude"), timer(F.codexPoll, "Codex")].filter(Boolean);
const pollSchedule = timers.length === 2
  ? `on a timer (${timers.join(", ")})`
  : timers.length === 1
    ? `on a timer (${timers[0]}) and otherwise at startup, on every page load and on Refresh now`
    : "at startup, on every page load and on Refresh now, with the poll timers off in the shipped config";
const pollBullet = (p, name, key, why) => p.on
  ? `${name} is polled every ${p.seconds} seconds per account${why}, set by \`${key}\` in \`usage/accounts.json\`; 0 turns the timer off, leaving one fetch at startup plus page loads and "Refresh now".`
  : `The shipped \`usage/accounts.json\` sets \`${key}\` to 0, so the ${name} timer is off: each ${name} account is fetched once at startup, then on every page load and on "Refresh now". A nonzero value polls on a timer at that many seconds; with the key removed the server uses ${p.defaultSeconds} seconds${why}.`;

const endpointRows = F.endpoints.map((e) => `| \`${e.method} ${e.host}${e.path}\` | ${e.what} |`).join("\n");

const md = `${MARKER}

${picture("masthead", `Harness Console: one local app for Harness Firmware repositories, with a usage tracker for every Claude Code and Codex account on this machine. The app runs on port ${F.appPort}, the tracker on port ${F.port}. The tracker reads ${F.claudeWindows.length} Claude windows and 2 Codex windows ${pollSchedule}, rescans token totals every ${F.tokenScanSeconds} seconds, and makes one write: the Claude token refresh.`)}
...
Every launch runs GitHub's latest main. The launcher first fetches \`origin/main\` and fast-forwards this folder, then starts the updated code. It skips the update and runs the copy on disk when the folder is on another branch, has uncommitted changes, holds commits GitHub does not, or GitHub is unreachable; the page footer shows the running build and, after a skip, the reason.

Every launch then starts fresh. A Harness Console already on port ${F.appPort} is asked to quit and is replaced, and any usage tracker on port ${F.port} (a \`node server.mjs\` answering \`/api/usage\`, including one started from an older checkout) is stopped and started again from \`usage/\`. When the running app is in the middle of an operation (clone, update, create, skill sync or DSH install), the launcher keeps it and only opens a new tab. Another service on port ${F.appPort} is left running and the launcher exits with an error; any other service answering on port ${F.port} is reused.
...
The app has five tabs (${F.tabs.join(", ")}) and opens on Usage. The sidebar lists every repository your GitHub account can reach, private ones included; its ↻ button reloads the list. Organization access depends on your GitHub authorization and any organization SSO requirements.

### Usage

Embeds the [usage tracker](#usage-tracker) from port ${F.port}. Until the tracker answers, the tab shows a waiting message.
...
Open http://127.0.0.1:${F.port}. The page rereads the tracker's cached snapshot every ${F.pageFetchSeconds} seconds and the reset countdowns tick every ${F.countdownSeconds}; "Refresh now" and every page load poll each account and rescan transcripts at once.
...
- ${pollBullet(F.claudePoll, "Claude", "claudePollSeconds", " (the usage endpoint rate-limits faster polling)")}
- ${pollBullet(F.codexPoll, "Codex", "codexPollSeconds", "")}
